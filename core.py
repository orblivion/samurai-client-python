...
# I made a mistake here. I should have made this more agnostic to tag
# names, and only looked at the type attribute and context/key in the
# case of messages. The consumer of the dict should take care of making
# it convenient for the consumer of the classes. Probably not worth
...
def _xml_outer_node_to_dict(xml_node):
    ...
    for outer_node in (node for node in xml_node.childNodes if node.nodeType == Node.ELEMENT_NODE):
        ...
        if element_name == 'payment_method':
            payment_method = _xml_outer_node_to_dict(
                outer_node)['payment_method']
            out_data['payment_method'] = payment_method
        elif element_name == 'processor_response':
            # the structure of 'processor_response' happens to be very
            # similar to the document at large. level 1, data. level 2,
            # messages so we'll just call this function recursively
            processor_response = _xml_outer_node_to_dict(
                outer_node)['processor_response']
            out_data['processor_success'] = processor_response.get(
                'success', False)
            for error in processor_response['errors']:
                out_data['errors'].append(
                    dict(error, source="processor"))  # it's set as a "samurai" error
            for info in processor_response['info']:
                out_data['info'].append(
                    dict(info, source="processor"))  # it's set as a "samurai" info
        # doc_element > messages > message
            ...
            for message in outer_node.getElementsByTagName("message"):
                ...
                if message.getAttribute('subclass') == "error":
                    out_data['errors'].append({
                        'context': message.getAttribute('context'),
                        'key': message.getAttribute('key'),
                        'source': "samurai"
                    })
            ...
            if outer_node.childNodes == []:
                ...
                for inner_node in (node for node in outer_node.childNodes if node.nodeType == Node.TEXT_NODE):
                    if element_type == 'integer':
                        ...
                        out_data[element_name] = datetime.strptime(
                            inner_node.nodeValue, "%Y-%m-%d %H:%M:%S UTC")
                    elif element_type == 'boolean':
                        out_data[element_name] = {
                            'true': True, 'false': False
                        }[inner_node.nodeValue]
                    else:
...
class RequestWithPut(urllib2.Request):
    ...
    def get_method(self):
        # can't use super, urllib2.Request seems to be old-style class
        super_method = urllib2.Request.get_method(self)
...
def _request(method, url, username, password, out_data={}):
    """
    Takes an input dictionary. For PUT, sends as XML payload to the
    supplied URL with the given method.

    For POST, sends as POST variables.

    Returns XML result as dictionary, accounting for FeeFighters'
    conventions, setting datatypes
    ...
    try:
        if method == "GET":
            ...
            opener = urllib2.build_opener(
                urllib2.HTTPSHandler(debuglevel=request_debugging))
        ...
        return {
            "error": {
                "errors": [{
                    "context": "client",
                    "source": "client",
                    "key": "http_error_response_" + str(e.code)
                }],
                "info": [],
            }
        }
...
class FeeFighters(object):
    """
    If you want to create multiple payment methods without repeating
    yourself with the authentication info, you can use this
    """
...
class RemoteObject(object):
    def _remote_object_request(self, request_name, url_parameter, payload={}, field_names=None):
        """
        This is a method that handles all requests, and should update
        the object's attributes with the new data
        """
    ...
    def _load_data_from_dict(self, in_data, field_names):
            ...
            in_data = {
                'error': {
                    'errors': [{
                        'source': 'client',
                        'context': 'client',
                        'key': 'wrong_head_element',
                    }],
                    'info': [],
                }
            }

        # check if we have all expected fields. if not, assume the whole thing
        # is a wash.
        ...
        # handle responses with the expected head element
            ...
            for field in field_names:
                setattr(self, field, in_data[
                        self.head_xml_element_name][field])
            ...
            for field in self.json_field_names:
                ...
                try:
                    setattr(self, field, json.loads(
                        in_data[self.head_xml_element_name][field]))
                except:
                    self.errors.append({
                        'source': 'client',
                        'context': 'client',
                        'key': 'json_decoding_error',
                    })

            self._last_data = in_data[
                self.head_xml_element_name]  # so we know what changed, for update()
...
class PaymentMethod(RemoteObject):
    ...
    updatable_field_names = [
        name for name in field_names if name not in [
            'custom', 'created_at', 'updated_at', 'errors', 'info'
        ]
    ]
    ...
    def __init__(self, *args, **kwargs):
        if 'feefighters' in kwargs and 'payment_method_initial' in kwargs:
            raise ValueError(
                "Can't supply both feefighters and payment_method_initial")

        if 'payment_method_initial' in kwargs:
            # we want to grab the token here, since this payment method will
            # probably be coming back from a transaction
            self._load_data_from_dict(
                kwargs['payment_method_initial'], self.field_names + ["payment_method_token"])
    ...
    def update(self):
        ...
        return self._remote_object_request(
...
class Transaction(RemoteObject):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('reference_id', None) == kwargs.get('payment_method', None) == None:
            raise ValueError(
                "Must supply either a reference_id or a payment_method")
        ...
        if kwargs.get('reference_id', None):   # pull up info for an existing transaction
            ...
            self.processor_token = kwargs[
                'processor_token']  # can be got from the transaction via fetch, if a reference_id is there
    ...
    def _transaction_request(self, request_name, url_parameter, payload={}, field_names=None):
        ...
        if self._remote_object_request(request_name, url_parameter, payload, field_names):
            if type(self.payment_method) == dict:
                self.payment_method = PaymentMethod(
                    payment_method_initial={'payment_method': self.payment_method}, merchant_key=self._merchant_key,
                ...
                self.errors.append({
                    "error": {
                        "errors": [{
                            "context": "client",
                            "source": "client",
                            "key": "errors_in_returned_payment_method"
                        }],
                        "info": [],
                    }
                })
            ...
            if type(self.payment_method) == dict:
                self.payment_method = PaymentMethod(
                    payment_method_initial={'payment_method': self.payment_method},
        ...
        if self.transaction_type:
            # so we don't get tripped up remembering "Purchase" vs "purchase"
            ...
            if getattr(self, field) != None:
                try:
                    out_data['transaction'][
                        field] = json.dumps(getattr(self, field))
                except:
                    self.errors.append({
                        "error": {
                            "errors": [{
    ...
    def purchase(self, amount, currency_code, billing_reference, customer_reference):  # default 'USD'?
        if self.reference_id:
            self.errors = [{
                "context": "client",
                "source": "client",
                "key": "attempted_purchase_on_existing_transaction",
            }]
            return False
            ...
            return {
                "error": {
                    "errors": [{
                        "context": "client",
                        "source": "client",
                        "key": "attempted_authorize_on_existing_transaction",
                    }],
                    "info": [],
                }
            }
    ...
    def capture(self, amount):  # default 'USD'?
        # in case we're rebuilding this transaction from a reference_id, and
        # they didn't want to fetch initially
        if self.transaction_token == None:
            self.fetch()

        new_transaction = Transaction(
            merchant_key=self._merchant_key, merchant_password=self._merchant_password,
        ...
        new_transaction._transaction_request(
            "capture_transaction", new_transaction.transaction_token, out_data)
    ...
    def void(self):
        ...
        new_transaction._transaction_request(
            "void_transaction", new_transaction.transaction_token)
    ...
    def credit(self, amount):
        ...
        new_transaction._transaction_request(
            "credit_transaction", new_transaction.transaction_token, out_data)
    ...
    def fetch(self):
        return self._transaction_request(
            ...
            self.reference_id,
            {},
            list(set(self.field_names) - set(['reference_id']))
        )
