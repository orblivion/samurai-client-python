...
def payment_method_redirect(request, update):
    ...
    if not new_payment_method.fetch() or not new_payment_method.is_sensitive_data_valid:
        return HttpResponseRedirect(
            reverse(error_redirect)
                + "?payment_method_token="
                + new_payment_method.payment_method_token
        )
    ...
    try:
        ...
        if update:
            old_payment_method = PaymentMethod.objects.get(
                payment_method_token=new_payment_method.custom['django_prev_payment_method_token']
            )
...
def get_transparent_redirect_form_initial(user, base_url, payment_method=None, update=False):
    ...
    if payment_method != None:
        # important to note that this may carry over custom set by the client
        # consumer, which we shouldn't change
        ...
        for key, value in payment_method_data.iteritems():
            ...
            init[key] = field
        init["custom"]['value'] = dict(init["custom"]['value'])
                                       # make a copy so I don't change the data
                                       # in the payment_method

        if update:
            init["payment_method_token"] = {
                'value': payment_method.payment_method_token}
                # making sure that the blank items (or "*****"'d secret items)
                # in the next generated PM are replaced with the values from
                # this PM
            if PaymentMethod.objects.filter(payment_method_token=payment_method.payment_method_token, user=user).exists():
                init["custom"]['value'][
                    "django_prev_payment_method_token"] = payment_method.payment_method_token  # passing on to the next payment method
            else:
                old_token_to_update = payment_method.custom.get(
                    'django_prev_payment_method_token', "not in DB")
                if PaymentMethod.objects.filter(payment_method_token=old_token_to_update, user=user).exists():
                    ...
                    init["custom"]['value'][
                        "django_prev_payment_method_token"] = old_token_to_update
                else:
                    update_url = False
    else:
        init["custom"] = {'value': {}}
    ...
    if update_url:
        init['redirect_url'] = {
            'value': base_url + reverse('samurai-update-payment-method-redirect')}
    else:
        init['redirect_url'] = {
            'value': base_url + reverse('samurai-new-payment-method-redirect')}
    ...
    if payment_method:
        if payment_method.last_four_digits:
            init['card_number']['value'] = "*" * \
                12 + payment_method.last_four_digits
            init['verification_value']['value'] = "*" * 3
        if not payment_method.is_sensitive_data_valid:
            init['verification_value'][
                'error'] = "Your credit card number or security code are invalid"
        ...
        if payment_method.last_transaction_error:
            ...
            init['non_field_error'] = (
                "There was an error with your information. Please look it over carefully and try again."
            )
