...
def _transparent_redirect(data):
    ...
    opener = urllib2.build_opener(
        NoRedirectHTTPRedirectHandler, urllib2.HTTPSHandler(debuglevel=request_debugging))
...
class TestXMLTODict(unittest.TestCase):
    ...
    def test_small_xml(self):
        ...
        self.assertEqual(
            out_dict, {'doc': {'a': '1', 'b': '2', 'errors': [], 'info': []}})

    def test_data_types(self):
        in_str = "<doc><a type='integer'>1</a></doc>"
        out_dict = _xml_to_dict(in_str)
        self.assertEqual(
            out_dict, {'doc': {'a': 1, 'errors': [], 'info': []}})

        in_str = "<doc><b type='boolean'>false</b><c type='boolean'>true</c></doc>"
        out_dict = _xml_to_dict(in_str)
        self.assertEqual(
            out_dict, {'doc': {'b': False, 'c': True, 'errors': [], 'info': []}})

        in_str = "<doc><d type='datetime'>2011-04-22 17:57:56 UTC</d></doc>"
        out_dict = _xml_to_dict(in_str)
        self.assertEqual(
            out_dict, {'doc': {'d': datetime(2011, 4, 22, 17, 57, 56), 'errors': [], 'info': []}})

        in_str = "<doc><d></d></doc>"
        out_dict = _xml_to_dict(in_str)
        self.assertEqual(
            out_dict, {'doc': {'d': "", 'errors': [], 'info': []}})
...
class TestDictToXML(unittest.TestCase):
    ...
    def test_dict_to_xml(self):
        ...
        self.assertTrue(xml.documentElement.getElementsByTagName(
            'last_name')[0].childNodes[0].nodeValue == "Fakerson")
        self.assertTrue(xml.documentElement.getElementsByTagName(
            'expiry_year')[0].childNodes[0].nodeValue == "2012")
...
class TestBasicRequest(unittest.TestCase):
    ...
    def test_fetch_payment_method_error(self):
        ...
        response = _request(
            request[0],
            request[1] % "badkey",
            test_credentials.merchant_key,
            test_credentials.merchant_password
        )
    ...
    def test_fetch_payment_method_success(self):
        ...
        response = _request(
            request[0],
            request[1] % payment_method_token,
            test_credentials.merchant_key,
            test_credentials.merchant_password
        )
        self.assertEqual(type(response), dict)

        self.assertEqual(response['payment_method'][
                         'payment_method_token'], payment_method_token)
        self.assertEqual(response['payment_method']['last_name'], "Fakerson")
        self.assertNotEqual(response['payment_method'][
                            'expiry_year'], "2012")  # make sure it's an int, not string.
        self.assertEqual(response['payment_method']['expiry_year'], 2012)
        self.assertEqual(
            response['payment_method']['address_1'], "123 Fake Street")

    def test_update_payment_method_error(self):
        "Test a bad PUT method"
        request = REQUESTS["update_payment_method"]

        response = _request(request[0], request[1] %
                            "badkey", test_credentials.merchant_key, test_credentials.merchant_password)
        self.assertTrue('error' in response)
    ...
    def test_update_payment_method_success(self):
        ...
        response = _request(request[0], request[1] % payment_method_token,
                            test_credentials.merchant_key, test_credentials.merchant_password, payload)
        self.assertEqual(type(response), dict)

        self.assertEqual(
            response['payment_method']['last_name'], "Actualperson")
        self.assertNotEqual(response['payment_method'][
                            'expiry_year'], "2018")  # make sure it's an int, not string.
        self.assertEqual(response['payment_method']['expiry_year'], 2018)
        self.assertEqual(
            response['payment_method']['address_1'], "321 Real Street")
...
feefighters = FeeFighters(
    merchant_key=test_credentials.merchant_key, merchant_password=test_credentials.merchant_password,
...
class TestPaymentMethodMethods(unittest.TestCase):
    ...
    def test_make_payment_method(self):
        ...
        try:
            payment_method = PaymentMethod(
                feefighters=feefighters, payment_method_token=payment_method_token, do_fetch=False)
        except:
            self.fail()

        self.assertEqual(
            payment_method._merchant_key, test_credentials.merchant_key)
        self.assertEqual(
            payment_method._merchant_password, test_credentials.merchant_password)
        self.assertEqual(
            payment_method.payment_method_token, payment_method_token)
    ...
    def test_do_fetch(self):
        ...
        payment_method_1 = PaymentMethod(
            feefighters=feefighters, payment_method_token=payment_method_token)
    ...
    def test_fetch(self):
        ...
        expected_none = [
            "created_at", "updated_at", "is_retained", "is_redacted", "is_sensitive_data_valid",
    ...
    def test_bad_fetch(self):
        ...
        self.assertEquals(payment_method.errors, [
                          {'source': 'client', 'context': 'client', 'key': 'http_error_response_404'}])

    def test_update(self):
        payment_method_token = _new_payment_method_token()
        payment_method = PaymentMethod(
            feefighters=feefighters, payment_method_token=payment_method_token, do_fetch=False)
        payment_method_check = PaymentMethod(
            feefighters=feefighters, payment_method_token=payment_method_token)
    ...
    def test_update_before_or_after_fetch(self):
        ...
        payment_method.custom = {
            'a': 'b'}  # handled somewhat differently, better test it
...
class TestTransactionMethods(unittest.TestCase):
    ...
    def test_make_transaction_with_payment_method(self):
        ...
        try:
            transaction = Transaction(
                feefighters=feefighters, payment_method=payment_method,
                                      processor_token=test_credentials.processor_token, do_fetch=False)
        except:
            self.fail()

        self.assertEqual(type(transaction.payment_method), PaymentMethod)
        self.assertEqual(
            transaction.payment_method.payment_method_token, payment_method_token)
        self.assertEqual(transaction.payment_method.first_name, None)
                         # do_fetch being False
    ...
    def test_make_transaction_credentials(self):
        ...
        self.assertEqual(
            transaction._merchant_key, test_credentials.merchant_key)
        self.assertEqual(
            transaction._merchant_password, test_credentials.merchant_password)
        self.assertEqual(
            transaction.processor_token, test_credentials.processor_token)
    ...
    def test_purchase(self):
        ...
        expected_none = ["reference_id", "transaction_token", "created_at",
                         "transaction_type", "amount", "currency_code", "processor_success"]
        ...
        for attr_name in expected_none:
            self.assertEqual(
                getattr(transaction, attr_name), None, attr_name + " not None")
        ...
        self.assertTrue(transaction.purchase(
            20.5, "USD", billing_reference, customer_reference), transaction.errors)

        for attr_name, var_val in expected_after.iteritems():
            self.assertEqual(getattr(transaction, attr_name), var_val)

        expected_info = {'source': 'processor', 'context':
                         'processor.transaction', 'key': 'success'}
        self.assertTrue(expected_info in transaction.info, str(
            expected_info) + " not in " + str(transaction.info))
    ...
    def test_bad_purchase(self):
        ...
        self.assertEqual(
            transaction.purchase(20.5, "USD", "12345", "321"), False)
    ...
    def test_bad_purchase_duplicate(self):  # this was based on a particular error I found.
        ...
        transaction.custom = {'a': 'b'}
        transaction.descriptor = {'c': 'd'}
        self.assertEqual(transaction.purchase(
            20.5, "USD", billing_reference, customer_reference), True)
        self.assertEqual(type(transaction.payment_method), PaymentMethod)
                         # make sure we don't get dicts in the payment method
                         # after bad purchases
        ...
        transaction.descriptor = {'c': 'd'}
        self.assertEqual(transaction.purchase(
            20.5, "USD", billing_reference, customer_reference), False)
        self.assertEqual(type(transaction.payment_method), PaymentMethod)
                         # make sure we don't get dicts in the payment method
                         # after bad purchases
        ...
        transaction.descriptor = {'c': 'd'}
        self.assertEqual(transaction.purchase(
            20.5, "USD", billing_reference + "0", customer_reference + "0"), True)
        self.assertEqual(type(transaction.payment_method), PaymentMethod)
                         # make sure we don't get dicts in the payment method
                         # after bad purchases
    ...
    def test_fetch(self):
        ...
        transaction_test = Transaction(
            feefighters=feefighters, reference_id=transaction.reference_id, do_fetch=False)

        for field_name in set(Transaction.field_names) - set(['info', 'errors', 'reference_id', 'descriptor', 'custom']):
            self.assertEqual(getattr(transaction_test, field_name), None,
                             field_name + " not None: " + str(getattr(transaction_test, field_name)))
        ...
        self.assertEquals(transaction_test.payment_method.payment_method_token,
                          transaction.payment_method.payment_method_token)
        self.assertEquals(
            transaction_test.payment_method.first_name, transaction.payment_method.first_name)
        self.assertEquals(
            transaction_test.payment_method.expiry_year, transaction.payment_method.expiry_year)
        self.assertEquals(
            transaction_test.payment_method.updated_at, transaction.payment_method.updated_at)

        for field_name in set(Transaction.field_names) - set(['errors', 'info', 'payment_method']):
            self.assertEqual(
                getattr(
                    transaction_test, field_name), getattr(
    ...
    def test_do_fetch(self):
        ...
        payment_method = PaymentMethod(
            feefighters=feefighters, payment_method_token=payment_method_token, do_fetch=False)
        transaction = Transaction(
            feefighters=feefighters, payment_method=payment_method, processor_token=test_credentials.processor_token)
        ...
        transaction_1 = Transaction(
            feefighters=feefighters, reference_id=transaction.reference_id)
    ...
    def test_authorize_capture(self):
        ...
        def check(t, t_type):
            ...
            self.assertEqual(
                t.payment_method.payment_method_token, payment_method_token)
            self.assertEqual(t.payment_method.first_name, "Nobody")

            self.assertTrue(t.populated)

            expected_info = {
                'source': 'processor', 'context': 'processor.transaction', 'key': 'success'}
            self.assertTrue(expected_info in transaction.info, str(
                expected_info) + " not in " + str(transaction.info))
        ...
        self.assertTrue(transaction.authorize(
            20.5, "USD", billing_reference, customer_reference), transaction.errors)
        ...
        check(transaction, "authorize")
              # confirm that it is still an "authorize" transaction
        self.assertNotEqual(
            transaction.reference_id, capture_transaction.reference_id)
                            # and that the ref IDs is different from the other
                            # txn
        self.assertEqual(ref_id, transaction.reference_id)
                         # but hasn't changed from the fetch
    ...
    def test_authorize_capture_from_fetched(self):
        ...
        self.assertTrue(transaction.authorize(
            20.5, "USD", "x" + billing_reference, "x" + customer_reference), transaction.errors)

        # Fetch a copy of the authorized transaction
        transaction_to_capture = Transaction(
            feefighters=feefighters, reference_id=transaction.reference_id)

        # Capture, confirm that the ref_id and txn_type changed during the
        # capture
        check(transaction_to_capture, "authorize")
        capture_transaction = transaction_to_capture.capture(20.5)
        self.assertNotEqual(
            transaction_to_capture.reference_id, capture_transaction.reference_id)
        check(capture_transaction, "capture")
    ...
    def test_credit(self):
        ...
        self.assertEquals(credit_transaction.errors, [
                          {'source': 'processor', 'context': u'processor.transaction', 'key': u'credit_criteria_invalid'}])
    ...
    def test_use_unfetched_transaction(self):
        ...
        capture_transaction = Transaction(
            feefighters=feefighters, reference_id=capture_transaction.reference_id, do_fetch=False)
...
class TestDjangoModels(unittest.TestCase):
    ...
    def test_sequence_not_through_db_1(self):
        ...
        PaymentMethod.objects.create(
            payment_method_token=_new_payment_method_token(), user=user)
        ...
        t = Transaction(
            payment_method=pm, processor_token=test_credentials.processor_token)
        ...
        pt_db = Transaction.objects.get(
            transaction_type="purchase", transaction_token=t.transaction_token)
        self.assertTrue(pt_db.amount == "20.0")
        self.assertTrue(pt_db.fetch())
        self.assertTrue(pt_db.amount == "20.0")

        vt_db = Transaction.objects.get(
            transaction_type="void", transaction_token=t.transaction_token)
        self.assertTrue(pt_db.amount == "20.0")
    ...
    def test_sequence_not_through_db_2(self):
        ...
        at_db = Transaction.objects.get(
            transaction_type="authorize", transaction_token=t.transaction_token)
        self.assertTrue(at_db.amount == "20.0")
        self.assertTrue(at_db.fetch())
        self.assertTrue(at_db.amount == "20.0")

        cat_db = Transaction.objects.get(
            transaction_type="capture", transaction_token=t.transaction_token)
        self.assertTrue(cat_db.amount == "18.0")
        self.assertTrue(cat_db.fetch())
        self.assertTrue(cat_db.amount == "18.0")

        crt_db = Transaction.objects.get(
            transaction_type="credit", transaction_token=t.transaction_token)
        self.assertTrue(crt_db.amount == "10.0")
        self.assertFalse(crt_db.fetch())
        self.assertTrue(crt_db.amount == "10.0")
        self.assertTrue(
            crt_db.errors == [{'source': 'processor', 'context': u'processor.transaction', 'key': u'credit_criteria_invalid'}])

        self.assertTrue(set([t.transaction_type for t in Transaction.objects.all()]) == set(
            ["credit", "authorize", "capture"]))
    ...
    def test_sequence_through_db_2(self):
        ...
        capture_txn = Transaction.objects.get(
            reference_id=capture_txn.reference_id)

        credit_txn = capture_txn.credit(10)
        self.assertTrue(credit_txn.errors == [
                        {'source': 'processor', 'context': u'processor.transaction', 'key': u'credit_criteria_invalid'}], credit_txn.errors)
        self.assertTrue(credit_txn.amount == "10.0")
        self.assertTrue(credit_txn.transaction_type == "credit")
        self.assertTrue(Transaction.objects.all().count() == 3)
    ...
    def test_migration(self):
        ...
        t.save()
               # t is now a transaction as we had it before adding these fields
