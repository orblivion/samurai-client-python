...
class PaymentMethod(models.Model, RemoteObject):
    ...
    last_transaction_error = models.BooleanField(
        default=False)  # stores whether the last transaction was an error
    disabled = models.BooleanField(default=False)

    def __init__(self, *args, **kwargs):
        super(PaymentMethod, self).__init__(*args, **kwargs)

        # Gonna do_fetch = False here. We don't want to fetch every
        # object every time we run PaymentMethod.objects.all() in the
        # shell. And we won't have control over fetching when we do a
        # filter.
        self._core_remote_object = CorePaymentMethod(
            feefighters=settings.SAMURAI_CREDENTIALS,
...
class Transaction(models.Model, RemoteObject):
    ...
    amount = models.CharField(
        ...
        blank=True,
        default="",
    )  # null and blank to avoid migration
    ...
    processor_success = models.NullBooleanField(
        editable=False,
        null=True,
        blank=True,
        default=None
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Gonna do_fetch = False here. We don't want to fetch every
        # object every time we run Transaction.objects.all() in the
        # shell And we won't have control over fetching when we do a
        # filter.
        if self.reference_id == None:
            self._core_remote_object = CoreTransaction(
                feefighters=settings.SAMURAI_CREDENTIALS,
                payment_method=self.payment_method._core_remote_object,
                processor_token=self.processor_token,
                do_fetch=False,
            )
        else:
            self._core_remote_object = CoreTransaction(
        ...
        # all we want are things that aren't in the database. Even if it's a
        # new transaction and the values are None, we want the attributes
        # there.
        self._get_fields_from_core(
            exclude=[
                "transaction_token",
    ...
    def _get_fields_from_core(self, exclude=[]):
        ...
        # self.payment_method should really always be set, but I'm avoiding
        # unnecessary errors if I made a mistake somewhere
    ...
    def _save_new_transaction(self, new_transaction_core):
        new_transaction = Transaction(
            transaction_token=new_transaction_core.transaction_token,
            reference_id=new_transaction_core.reference_id,
            payment_method=self.payment_method,
            transaction_type=new_transaction_core.transaction_type,
            amount=new_transaction_core.amount,
            currency_code=new_transaction_core.currency_code,
            created_at=new_transaction_core.created_at,
    ...
    def purchase(self, amount, currency_code, billing_reference, customer_reference):
        self._set_fields_into_core()
        result = self._core_remote_object.purchase(
            amount, currency_code, billing_reference, customer_reference)
        self._get_fields_from_core()
        try:
            self.full_clean()
        except:
    ...
    def authorize(self, amount, currency_code, billing_reference, customer_reference):
        self._set_fields_into_core()
        result = self._core_remote_object.authorize(
            amount, currency_code, billing_reference, customer_reference)
        self._get_fields_from_core()
        try:
            self.full_clean()
